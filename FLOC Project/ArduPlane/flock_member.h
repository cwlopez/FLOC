...
class flock_member
{
 private:
	 ...
	 int32_t	_relative_alt;		//Altitude above ground in meters, expressed as *100 (mm)
	 ...
	 //english equivalents
	 int32_t	_alt_ft;			//Altitude in feet, expressed as *1000 , above MSL (3 decimal precision)
	 int32_t	_relative_alt_ft;	//Altitude above ground in feet, expressed as *1000 (3 decimal precision)
	 Vector3ui	_V_fps;				//Ground Speed (GPS frame), exressed as ft/s*100 (2 decimal precision)
	 ...
	 //relative positions
	 Vector3l	_dX[FLOCK_SIZE];	//Distance between a/c and other flock members (NED frame) if ft*100 (2 decimal precision) 	
	 
	 //velocities relative to leader
	 Vector3l	_dV;				//Ground speed (NED frame) difference between a/c and leader, expressed as ft/s *100 (2 decimal precision)
	 Relative	_my_relative;		//Structure containing all the relative S/A information
	 
	 //distance magnitude from leader
	 int32_t	_dist_2_leader;		//Magnitude of distance between a/c and leader, expressed as ft *100 (2 decimal precision)
 public:
	...
	void set_state(int32_t* p_current_lat, int32_t* p_current_lon, int32_t* p_current_alt, int32_t* p_current_relative_alt, 
